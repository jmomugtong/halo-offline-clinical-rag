...
This work is motivated in part by KaagapAI\footnote{\url{https://github.com/jmomugtong/KaagapAI}}, an open-source clinical AI assistant developed by the author for remote rural health units in the Philippines, where internet connectivity is intermittent or absent, GPU hardware is unavailable, and patients often lack access to specialist referral. KaagapAI operationalizes exactly the deployment scenario studied here---offline, CPU-only, privacy-preserving clinical question answering---and the retrieval architecture choices examined in this paper were selected precisely because they are feasible in that environment. The null results reported below are therefore not merely an academic curiosity but carry direct practical implications: they suggest that the dominant retrieval configurations used in offline clinical AI applications today provide equivalent hallucination rates, and that improving answer fidelity in such settings will require advances beyond retrieval architecture alone.
...
Table~\ref{tab:related} summarises representative prior work along the four axes most relevant to HALO: retrieval architecture, evaluation benchmark, primary metric, and offline (CPU-free) deployability. The table makes the source of surprise explicit: prior work consistently reports retrieval improvements measured via retrieval-centric metrics (MRR, nDCG, EM) on general-domain benchmarks with GPU-resident systems. HALO is the first evaluation to measure the \emph{downstream hallucination} effect of the full BM25-to-hybrid-to-reranked progression on a clinical benchmark under strict CPU-only constraints, with faithfulness-NLI as the primary metric. The gap between the retrieval-MRR gains reported in prior work and the flat faithfulness-NLI results reported here is precisely what motivates the metric saturation and TF-IDF approximation diagnoses in Section~\ref{sec:discussion}.
...
\toprule
\textbf{Work} & \textbf{Retrieval} & \textbf{Benchmark (domain)} & \textbf{Primary metric} & \textbf{CPU-only} & \textbf{Finding relevant to HALO} \\
\midrule
Lewis et al.\ \cite{lewis2020retrieval}      & Dense (DPR)                   & NQ, TriviaQA (general)       & EM                     & No           & RAG outperforms closed-book; dense $>$ BM25 on EM \\
Karpukhin et al.\ \cite{karpukhin2020dense}  & Dense (bi-encoder)            & NQ, TriviaQA (general)       & Top-20 recall          & No           & Dense bi-encoder substantially outperforms BM25 recall \\
Cormack et al.\ \cite{cormack2009reciprocal} & Hybrid (RRF)                  & TREC (general)               & MAP, nDCG              & Yes          & RRF improves over individual rankers without supervision \\
Nogueira \& Cho \cite{nogueira2019passage}   & BM25 + cross-encoder          & MS-MARCO (general)           & MRR@10                 & No           & BERT reranking substantially improves BM25 precision \\
Thakur et al.\ \cite{thakur2021beir}         & Sparse + Dense (zero-shot)    & BEIR incl.\ BioASQ (multi)   & nDCG@10                & Sparse only  & No single retriever dominates; biomedical domain shows largest gaps \\
Singhal et al.\ \cite{singhal2023large}      & None (parametric)             & MultiMedQA (clinical)        & Accuracy               & No           & LLMs encode clinical knowledge; grounding required for safety \\
Labrak et al.\ \cite{labrak2024biomistral}   & Dense (BioMistral embeds)     & BioASQ, MedQA (clinical)     & Accuracy               & Partial      & Domain pretraining improves clinical QA; retrieval adds accuracy \\
\midrule
\textbf{HALO (this work)}                    & BM25 / hybrid / reranker (12 cond.) & BioASQ Task B (clinical) & Faithfulness-NLI, EM-F1 & \textbf{Yes} & All 12 conditions statistically equivalent; metric saturates on factoid answers \\
\bottomrule
\end{tabular}
}
\caption{Representative prior retrieval and clinical RAG work compared on axes relevant to HALO. Prior work reports improvements via retrieval-centric metrics on general-domain benchmarks with GPU-dependent systems. HALO is the first CPU-only ablation measuring downstream faithfulness-NLI across a full sparse-to-hybrid-to-reranked condition range on a clinical benchmark.}
\label{tab:related}
\end{table}
...
Algorithm~\ref{alg:halo} presents the pseudocode for the full HALO retrieval pipeline.

\begin{algorithm}[t]
\caption{HALO Retrieval Pipeline}
\label{alg:halo}
\begin{algorithmic}[1]
\REQUIRE query $q$, corpus $\mathcal{C}$, parameters $k$, $n_1$, $n_2$, $n_3$, flag \texttt{reranker\_active}
\ENSURE context window $C^*$
\STATE $L_{\text{sparse}} \leftarrow \text{BM25Retrieve}(q, \mathcal{C}, \text{top\_n}=n_1)$ \COMMENT{Sparse stage}
\STATE $L_{\text{dense}}  \leftarrow \text{TFIDFFAISSRetrieve}(q, \mathcal{C}, \text{top\_n}=n_1)$ \COMMENT{Dense stage}
\STATE $L_{\text{fused}}  \leftarrow \text{RRF}(L_{\text{sparse}}, L_{\text{dense}}, k=k)[:n_2]$ \COMMENT{Fusion stage}
\IF{\texttt{reranker\_active}}
  \STATE $\text{scores} \leftarrow \text{CrossEncoder}(q, L_{\text{fused}})$ \COMMENT{Rerank stage}
...
\begin{tabular}{llllll}
\toprule
\textbf{Abbrev.} & \textbf{Sparse} & \textbf{Dense} & \textbf{RRF $k$} & \textbf{Reranker} & \textbf{Prompt Style} \\
\midrule
BM25-Def    & BM25 (def.) & ---           & ---      & No  & Unstructured   \\
TFIDF-FAISS & ---         & TF-IDF+FAISS  & ---      & No  & Unstructured   \\
RRF-NR      & BM25 (def.) & TF-IDF+FAISS  & 60       & No  & Unstructured   \\
BM25-GA     & BM25 (GA)   & ---           & ---      & No  & Unstructured   \\
HALO        & BM25 (def.) & TF-IDF+FAISS  & 60       & Yes & Unstructured   \\
Ref-BM25    & BM25 (def.) & ---           & ---      & No  & Refusal-cond.  \\
Ref-Hyb     & BM25 (def.) & TF-IDF+FAISS  & 60       & Yes & Refusal-cond.  \\
Cit-BM25    & BM25 (def.) & ---           & ---      & No  & Citation-anch. \\
Cit-Hyb     & BM25 (def.) & TF-IDF+FAISS  & 60       & Yes & Citation-anch. \\
Unstr-BM25  & BM25 (def.) & ---           & ---      & No  & Unstructured   \\
Unstr-Hyb   & BM25 (def.) & TF-IDF+FAISS  & 60       & Yes & Unstructured   \\
Adpt-Rtr    & Adaptive    & Adaptive      & Adaptive & No  & Unstructured   \\
\bottomrule
\end{tabular}
\caption{Hyperparameter configuration for all 12 HALO ablation conditions. All conditions share $n_1=50$, $n_2=20$, $n_3=5$, temperature $= 0.0$, max context $= 2048$ tokens, and seeds $\{42, 123, 456, 789, 1024\}$.}
\label{tab:1}
\end{table}
